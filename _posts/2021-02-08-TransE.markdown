...
다음과 같은 식은 Objective Function을 의미한다.  

$$\mathit{d(h+l,t)}$$

 : Dissimilarity measure - $L_{1}/L_{2}$ 가능  
 여기서 $\hat{h},l,\hat{t}\in S^{+}$ 은 **Corrupted Triplet**을 의미하여 $S^{+}_{h,l,t} = \left \{ (h^{+}, l, t)|h^{+}\in E \cup  (h,l,t^{+})|t^{+}\in E \right \}$ 를 의미한다. 여기서 해석한 바로는 Head와 Tail을 제외한 나머지 Entity를 의미한다고 해석했다.  
