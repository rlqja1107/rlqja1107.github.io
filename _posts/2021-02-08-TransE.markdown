...
다음과 같은 식은 *Objective Function*을 의미한다.  

$$\mathit{d(h+l,t)}$$  

이는 Dissimilarity measure을 나타내고, $L_{1}/L_{2}$ 가능하다.    
$$\hat{h},l,\hat{t}\in S^{+}$$ 
...
Corrupted Triplet에서 head, tail, relation의 영역은 위와 같이 나타낸다. 여기서 해석한 바로는 Sampling(?)된 Head와 Tail을 제외한 나머지 Entity의 집합을 의미한다고 해석했다.  
